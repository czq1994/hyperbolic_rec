...
from model.hyperbolic_model1 import Model, Controller
...
def parse_args():
    ...
    parser.add_argument('--data', type=str, default='ml20m')
        ...
        try:
            for t in range(self.config.epoch):
                ...
                for batchID in range(num_batches):
                    ...
                    user_emb = self.model.user_hb_embeddings[user_id].view(-1, self.config.hidden_dim)
                    pos_emb = self.model.item_hb_embeddings[pos_id].view(-1, self.config.hidden_dim)
                    neg_emb = self.model.item_hb_embeddings[neg_id].view(-1, self.config.hidden_dim)
